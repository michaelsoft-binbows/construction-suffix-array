...
// 
// All modifications are marked as 'modification'.
// 

// An implementation of the "Suffix Array Induced Sorting" algorihtm by Nong et al.
vector<int> modified_construct(vector<int> text, int rec) {
    ...
    // Phase 1
    {
        ...
        // Step I
        {
            ...
            // Modification
            if (rec == 0) for (int i = 0; i < dollar.size(); ++i) sa[i+1] = dollar[i];
        }
        ...
        // Step II
        {
            ...
            // Modification
            for (int i = (rec == 0 ? 1 : 0), j, k; i < n; ++i) if (sa[i] != -1) {
        ...
        // Step V
        {
            ...
            else sa_new = modified_construct(text_new, rec+1); // recursion
